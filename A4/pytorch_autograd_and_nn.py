...
def flatten(x, start_dim=1, end_dim=-1):
  """
  @deprecated Please use nn.Flatten!
  """
...
def three_layer_convnet(x, params):
  ...
  scores = F.linear(torch.flatten(x, start_dim=1), fc_w, bias=fc_b)
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    #       You don't have to use OrderedDict.                                    
    # Inputs:                                                                  
    ...
    self.net = nn.Sequential(
      nn.BatchNorm2d(Cin), # Spatial Batch Norm
      nn.ReLU(),
      nn.Conv2d(Cin, Cout, kernel_size=3, padding=1, stride=stride),
      nn.BatchNorm2d(Cout), # Spatial Batch Norm
      nn.ReLU(),
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    for in_channel, out_channel, num_blocks, do_downsample in stage_args:
      blocks.append(ResNetStage(in_channel, out_channel, num_blocks, downsample=do_downsample, block=block))
    ...
    self.fc = nn.Sequential(nn.Flatten(), 
                            nn.Linear(stage_args[-1][1], num_classes))
  ...
  def forward(self, x):
    ...
    # To avoid dependency on the size of the input, ResNet has an average pooling at 
    # the end of the convolutional part, such that the size of the input tensor to 
    # the linear layer is always (batch_size, stage_args[-1][1]) 
    _, _, H, W = x.size()
    x = F.avg_pool2d(x, (H, W)) # (N x stage[-1][1])
